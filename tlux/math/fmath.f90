...
CONTAINS
  ...
  SUBROUTINE SVD(AT, S, VT, RANK, STEPS, BIAS)
    IMPLICIT NONE
    REAL(KIND=RT), INTENT(IN), DIMENSION(:,:) :: AT
    REAL(KIND=RT), INTENT(OUT), DIMENSION(SIZE(AT,1)) :: S
    REAL(KIND=RT), INTENT(OUT), DIMENSION(SIZE(AT,1),SIZE(AT,1)) :: VT
    INTEGER, INTENT(OUT), OPTIONAL :: RANK
    INTEGER, INTENT(IN), OPTIONAL :: STEPS
    REAL(KIND=RT), INTENT(IN), OPTIONAL :: BIAS
    ! Local variables.
    REAL(KIND=RT), DIMENSION(SIZE(AT,1),SIZE(AT,1)) :: ATA, Q
    ...
    CALL SSYRK('U', 'N', K, SIZE(AT,2), MULTIPLIER**2, AT(:,:), &
         SIZE(AT,1), 0.0_RT, ATA(:,:), K)
