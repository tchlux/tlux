...
CONTAINS
  ...
  RECURSIVE SUBROUTINE BUILD_TREE(POINTS, SQ_SUMS, RADII, MEDIANS, &
       & INNERS, SQ_DISTS, ORDER, ROOT, LEAF_SIZE, SELECTOR)
    ...
    REAL(KIND=R32), INTENT(OUT), DIMENSION(:) :: INNERS
    ...
    IF (PRESENT(SELECTOR)) THEN
       ...
       END IF
    ELSE
       ! SELECTOR_METHOD = SELECT_FURTHEST_POINT  ! Default, inner child is furthest.
       SELECTOR_METHOD = SELECT_NEAREST_POINT  ! Default, inner child is nearest.
    END IF
    ...
    ELSE
       IF (SELECTOR_METHOD .EQ. SELECT_RANDOM_POINT) THEN
          ...
          CENTER_IDX = 1_I64 + INT(RANDOM_FLOAT * REAL(SIZE(ORDER,KIND=I64)))
    ...
    ! Store the "radius" of this ball, distnace to the furthest point.
    RADII(ORDER(1)) = SQRT(SQ_DISTS(I))
    ! Chose the roots of the children.
    IF (SELECTOR_METHOD .EQ. SELECT_RANDOM_POINT) THEN
       ! Pick the inner root.
       CALL RANDOM_NUMBER(RANDOM_FLOAT)
       I = 1_I64 + INT(RANDOM_FLOAT * REAL(MID), KIND=I64)
       CALL SWAP_I64(ORDER(2), ORDER(I))
       INNERS(ORDER(1)) = SQRT(SQ_DISTS(ORDER(2)))
       ! Pick the outer root.
       CALL RANDOM_NUMBER(RANDOM_FLOAT)
       I = MID+1_I64 + INT(RANDOM_FLOAT * REAL(SIZE(ORDER,KIND=I64)-MID), KIND=I64)
    ...
    ELSE IF (SELECTOR_METHOD .EQ. SELECT_NEAREST_POINT) THEN
       ...
       CALL SWAP_I64(ORDER(2), ORDER(I))
       ! Compute the distance to the inner child (useful for radius queries).
       INNERS(ORDER(1)) = SQRT(SQ_DISTS(ORDER(2)))
    ELSE
       ! Move the median point (furthest "interior") to the front (inner root).
       CALL SWAP_I64(ORDER(2), ORDER(MID))
       ! The distance to the inner is the median distance, already computed above.
       INNERS(ORDER(1)) = MEDIANS(ORDER(1))
       ! Move the furthest point into the spot after the median (outer root).
       CALL SWAP_I64(ORDER(MID+1_I64), ORDER(I))
    END IF
    ...
    CALL BUILD_TREE(POINTS, SQ_SUMS, RADII, MEDIANS, INNERS, SQ_DISTS(2_I64:MID), ORDER(2_I64:MID), 1_I64, LS, SELECTOR_METHOD)
    !$OMP SECTION
    !   Only perform this operation if there are >0 points available.
    IF (MID < SIZE(ORDER,KIND=I64)) THEN
       CALL BUILD_TREE(POINTS, SQ_SUMS, RADII, MEDIANS, INNERS, SQ_DISTS(MID+1_I64:), ORDER(MID+1_I64:), 1_I64, LS, SELECTOR_METHOD)
    END IF
    !$OMP END SECTIONS
    !$OMP END PARALLEL
  ...
  ! Find all points within radius R of POINT.
  RECURSIVE SUBROUTINE PT_IN_RADIUS(POINT, RADIUS, TREE, SQ_SUMS, RADII, MEDIANS, &
       INNERS, ORDER, LEAF_SIZE, INDICES, DISTS, PT_SS, D_ROOT)
    REAL(KIND=R32), INTENT(IN), DIMENSION(:) :: POINT
    REAL(KIND=R32), INTENT(IN), DIMENSION(:,:) :: TREE
    REAL(KIND=R32), INTENT(IN), DIMENSION(:) :: SQ_SUMS
    REAL(KIND=R32), INTENT(IN), DIMENSION(:) :: RADII
    REAL(KIND=R32), INTENT(IN), DIMENSION(:) :: MEDIANS
    REAL(KIND=R32), INTENT(IN), DIMENSION(:) :: INNERS
    INTEGER(KIND=I64), INTENT(IN), DIMENSION(:) :: ORDER
    INTEGER(KIND=I64), INTENT(IN) :: LEAF_SIZE
    REAL(KIND=R32), INTENT(IN) :: RADIUS
    INTEGER(KIND=I64), INTENT(OUT), DIMENSION(:) :: INDICES
    REAL(KIND=R32), INTENT(OUT), DIMENSION(:) :: DISTS
    REAL(KIND=R32), INTENT(IN), OPTIONAL :: PT_SS
    REAL(KIND=R32), INTENT(IN), OPTIONAL :: D_ROOT
    ! Local variables
    INTEGER(KIND=I64) :: I, I1, I2, MID
    REAL(KIND=R32) :: D, D0, D1, D2, PS
    ! Initialize FOUND for first call, if FOUND is present then
    ! this must not be first and all are present.
    INITIALIZE : IF (PRESENT(PT_SS)) THEN
       PS = PT_SS
       D0 = D_ROOT
    ELSE IF (SIZE(ORDER) .GT. 0) THEN ! There is at least one point to check, this is root.
       ! Start at index 0 (added onto current index). Compute squared sum.
    ...
    BRANCH_OR_LEAF : IF (SIZE(ORDER) .GT. LEAF_SIZE) THEN
       ! Measure distance to inner child.
       I1 = ORDER(2)
       ! Only compute the distance if it might be within the radius.
       ! IF (D0 - INNERS(I1) .LE. RADIUS) THEN
          D1 = SQRT(PS + SQ_SUMS(I1) - &
               2*DOT_PRODUCT(POINT(:),TREE(:,I1)))
       ! ELSE
       !    D1 = D0 - INNERS(I1)
       ...
       ELSE
          I2 = ORDER(MID+1)
          IF (I2 .NE. I1) THEN
             ! IF (D0 - RADII(ORDER(1)) .LE. RADIUS) THEN
                D2 = SQRT(PS + SQ_SUMS(I2) - &
                     2*DOT_PRODUCT(POINT(:),TREE(:,I2)))
             ! ELSE
             !    D2 = D0 - RADII(ORDER(1))
       ...
       END IF
       ! Search the inner child if it could contain a nearer point.
       SEARCH_INNER : IF (D1 .LE. RADIUS + RADII(I1)) THEN
          CALL PT_IN_RADIUS(POINT, RADIUS, TREE, SQ_SUMS, RADII, MEDIANS, &
               INNERS, ORDER(2:MID), LEAF_SIZE, INDICES, DISTS, PS, D1)
       END IF SEARCH_INNER
       ! Search the outer child if it could contain a nearer point.
       SEARCH_OUTER : IF ((I2 .GT. 0) .AND. (I2 .NE. I1)) THEN
          IF (D2 .LE. RADIUS + RADII(I2)) THEN
             CALL PT_IN_RADIUS(POINT, RADIUS, TREE, SQ_SUMS, RADII, MEDIANS, &
                  INNERS, ORDER(MID+1:), LEAF_SIZE, INDICES, DISTS, PS, D2)
          END IF
       END IF SEARCH_OUTER
    ! Since this is a leaf node, we measure distance to all children.
    ELSE
       ...
       DIST_TO_CHILDREN : DO I = 2, SIZE(ORDER)
          ...
          ! Store this distance.
          DISTS(ORDER(I)) = D
          INDICES(ORDER(I)) = ORDER(I)
       END DO DIST_TO_CHILDREN
    END IF BRANCH_OR_LEAF
    ! Handle closing operations..
    FIND_IN_RADIUS : IF (.NOT. PRESENT(PT_SS)) THEN
       PRINT *, "TODO: Find all values in the list less than specified value."
       ! ! This is the root, initial caller. Sort the distances for return.
       ! CALL ARGSELECT(DISTS(:F), INDICES(:F), K)
       ! CALL ARGSORT(DISTS(:K), INDICES(:K))
       ! ! Set all unused dists and indices to default values.
       ! INDICES(F+1:) = 0
       ! DISTS(F+1:) = HUGE(DISTS(1))
    END IF FIND_IN_RADIUS
  END SUBROUTINE PT_IN_RADIUS



  ! Reorganize a built tree so that it is packed in order in memory.
  SUBROUTINE FIX_ORDER(POINTS, SQ_SUMS, RADII, MEDIANS, INNERS, ORDER, COPY)
    REAL(KIND=R32),   INTENT(INOUT), DIMENSION(:,:) :: POINTS
    ...
    REAL(KIND=R32),   INTENT(INOUT), DIMENSION(:) :: INNERS
    INTEGER(KIND=I64), INTENT(INOUT), DIMENSION(:) :: ORDER
    ...
    ELSE ; SHOULD_COPY = PRODUCT(SHAPE(POINTS)) .LE. MAX_COPY_BYTES
    END IF
