...
- [ ] Close the random language gate. The fixed 40-case Fourth Wing baseline now reaches evidence recall@5/precision@1/MRR of 0.850/0.675/0.743 after contraction-aware, signal-ranked clue generation. Model refinement is capped at four lanes while deterministic recovery retains seven; the latest eight-case Gemma 3 smoke reaches 1.000/0.875/0.938, so larger precision and conditional-query gates remain open.
...
- [ ] Close the conditional language gate. The grounded seven-case fixture in `plan/benchmark_language_conditionals.md` passes 7/7 deterministic group-coverage/coherence checks at top-k 5 and the latest run measures 0.929 judged precision over 11 labels; LQ-16 and LQ-17 still expose early antipatterns, so unknown-hit precision remains open.
