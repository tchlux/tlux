...
- [x] Verify persistent JSONL service throughput on 101 independent random passages: 101/101 grounded responses, zero errors, and 20.8/60.2 ms median/p95 request latency including full evidence validation.
- [x] Reuse one lazy HTTP/1.1 connection for LM Studio planner calls and reconnect once after a dropped socket; the independent 20-sample Gemma 4 gate retained perfect evidence metrics at 646/938 ms median/p95.
- [x] Tighten evidence acceptance to require the normalized raw passage, or all of at least four distinct request terms after punctuation/word-order normalization, in readable sources, then rank verified evidence before lexical score. Partial generic overlap remains rejected; all 395 prose and all 75 repository passages retain 1.000 evidence recall/precision@1/MRR.
...
- [ ] Close the random language gate. The fixed 80-case Fourth Wing baseline now reaches evidence recall@5/precision@1/MRR of 1.000/0.9625/0.9813; vague and conditional precision@1 are 0.950, specific is 1.000, and missing-entity is 0.950. A broader exploratory 200-case slice reaches 0.965/0.890/0.9225, so generic random-clue recall remains open. `--always-refine` forces an auditable LM inspection round.
- [x] Add a bounded per-index language-lane cache. Repeated-query smoke preserved result IDs while reducing search time from 4.91 seconds to 0 ms and agent time from 5.01 seconds to 89 ms on the second request.
- [ ] Close the conditional language gate. The grounded seven-case fixture in `plan/benchmark_language_conditionals.md` passes 7/7 deterministic group-coverage/coherence checks at top-k 5 and 1.000 judged precision over 10 labeled hits. Broader random conditional precision and unknown-hit coverage remain open.
- [x] Gate the expanded heterogeneous language corpus. The valid 512-file FineWeb profile index now passes a 32-case Gemma 3 model-first slice at 1.000 recall/precision/MRR across vague, specific, conditional, and missing-entity styles, with 31/32 model queries and one validated rescue at 5.94/6.34 s median/p95 agent latency.
- [x] Add a 20-case heterogeneous FineWeb language gate over unrelated domains, conditionals, and forgotten entities; it passes 20/20 coverage/coherence and 1.000 judged precision over 40 labels at top-k 5.
...
returned source must contain the sampled passage after whitespace normalization,
or all of at least four distinct request terms after punctuation/word-order
normalization. Partial generic overlap is rejected. If the source snapshot is
unavailable, the audit accepts at least 90% of distinct terms. This separates a
genuinely relevant duplicate file from an exact document-id miss and keeps the
target-id audit visible.
...
- 2026-07-14: Raised language-memory generation to 32 output tokens, weighted condition coverage more strongly, protected focused first-pass hits during model refinement, and removed one-word clause lanes. The fixed random baseline is now 0.725/0.450/0.571 recall/precision/MRR with 650 ms median latency; a 20-case Gemma 3 model-first run reaches 1.000/0.800/0.888 with 3.44 s median latency.
- 2026-07-14: Added a focused missing-entity content lane and enforced omission of the first sampled clue in LM benchmark queries. The fixed 40-case baseline improved to 0.775/0.550/0.646 recall/precision/MRR; the FineWeb 32-case deterministic slice measured 0.906/0.750/0.807, exposing generic-term failures for the next rarity-stratified gate.
- 2026-07-14: Passed the expanded FineWeb model-first language gate: 32/32 rank-one evidence results across all four styles, 31/32 genuine Gemma 3 queries, and one validated deterministic rescue at 5.94/6.34 s median/p95 agent latency.
- 2026-07-14: Raised deterministic language coverage weighting to 0.50, added targeted explicit-contrast parsing, and redacted the omitted clue in missing-entity LM prompts. The fixed Fourth Wing 40-case slice now measures 1.000/0.900/0.935 recall/precision/MRR; the seven-case conditional fixture reaches 1.000 judged precision, and the checked-in 10-case FineWeb challenge fixture passes 1.000 coverage and precision over 20 labels.
- 2026-07-14: Raised language coverage weighting to 0.75, added a bounded 60-hit lexical rescue page, staged deterministic first-pass refinement, and added a semantic companion for missing-memory clues. The 80-case Fourth Wing slice now measures 1.000/0.9625/0.9813 recall/precision/MRR; the 20-case extended FineWeb fixture passes 1.000 coverage and judged precision over 40 labels.
