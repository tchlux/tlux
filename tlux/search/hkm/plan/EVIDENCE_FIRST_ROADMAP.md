...
- 2026-07-14: Hardened the model-first planner-tool lane: generic instruction words no longer satisfy the evidence guard, truncated JSON query strings are parsed safely, planner output is bounded, the default LM Studio timeout is five seconds, and lexical rescue stops once rank-1 evidence is proven. Current 50-prose and 75-repository live gates remain perfect on evidence quality with one model call per sample.
