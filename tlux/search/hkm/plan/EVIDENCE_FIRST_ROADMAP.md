...
- [x] Add the structured planner-to-tool agent path. With deterministic-first routing, all 395 prose passages and all 75 repository passages retain 1.000 evidence recall/precision@1/MRR with zero model calls; median/p95 latency is 256/361 ms on prose and 92/954 ms on the repository.
...
- [ ] Reduce the remaining tail latency. Candidate-first lexical ranking now measures 254/361 ms median/p95 on all 395 prose chunks with perfect evidence quality; model-first planner-tool calls remain about 0.95/1.37 s median/p95, so local model generation is still the dominant cost.
...
- 2026-07-14: Hardened the model-first planner-tool lane: generic instruction words no longer satisfy the evidence guard, truncated JSON query strings are parsed safely, planner output is bounded, the default LM Studio timeout is five seconds, lexical rescue stops once rank-1 evidence is proven, and ungrounded candidates fail closed. Current 50-prose and 75-repository live gates remain perfect on evidence quality with one model call per sample.
