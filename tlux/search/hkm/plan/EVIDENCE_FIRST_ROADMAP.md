...
- [x] Wire deterministic-first routing into the tool path. The same 50-sample gates retained 1.000 content-evidence recall while reducing model calls to 2% on prose (about 361 ms median result latency) and 10% on the repository (about 165 ms).
...
- 2026-07-13: Added token-only tool retrieval, a one-completion grounded result path, bounded model queries, confidence-triggered semantic/lexical fallback lanes, and deterministic-first routing. A 100-sample live cross-corpus gate retained 1.000 content-evidence recall; deterministic-first reduced median result latency to about 165-361 ms with 2-10% model calls.
