...
- [x] Add explicit tool search modes. On the same five-passage live gate, token-only retrieval kept tool evidence recall/precision at 1.000 and reduced median HKM search time from about 476 ms to 336 ms; hybrid remains the default pending a larger gate.
...
- 2026-07-13: Added token-only tool retrieval as an explicit latency experiment; the five-sample live gate retained perfect evidence metrics while reducing median HKM search time by about 29%.
